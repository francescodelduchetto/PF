...
#                dist_2 = np.sum((np.asarray(coords) - np.asarray(node))**2, axis=1) #distances between GPS location and all nodes
                ...
                for j in range(0,len(state['Particles'])):    #Iterate over number of pickers
                    ...
                    times = nodes_occuppied[:,1].astype(np.float)                            # Number of particles in Xth node  (change time to times)
                    D=[]
...
#                    print W
                    ...
                    if Neff < No_of_ptcl/10:
                        W = [1/No_of_ptcl]*No_of_ptcl
                
#                    belief=[]
##                    #########################################################
#                    for ch in range(0,len(times)):
#                        if times[ch] > Neff:
#                            belief.append(W[ch] * Neff)
#                        else:
#                            belief.append(0)
#                    est = np.asarray(belief).argmax()
#                    #########################################################
                    est = (W*times).argmax()               # weight x number of particles in a node  
#                    state['Particles'][j] = resample(W,state['Particles'][j])
